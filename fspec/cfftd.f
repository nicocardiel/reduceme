C------------------------------------------------------------------------------
C Version 04-December-2007                                        File: cfftd.f
C------------------------------------------------------------------------------
...
Comment
C
C SUBROUTINE CFFTD(N,XR,XI,IMODE)
C
...
C DOUBLE PRECISION XR(N) -> real part of the input data
C DOUBLE PRECISION XI(N) -> imaginary part of the input data
C INTEGER IMODE -> +1 (direct FFT) or -1 (inverse FFT)
C
Comment
C------------------------------------------------------------------------------
! This function is identical to CFFT, but using arguments in double precision
        SUBROUTINE CFFTD(N,XR,XI,IMODE)
        IMPLICIT NONE
C
        INTEGER N
        DOUBLE PRECISION XR(N),XI(N)
        INTEGER IMODE
C local variables
        INTEGER NU,N0
        INTEGER N2,NU1,K,L,I,P,K1,K1N2
        INTEGER IBITRFFT
        DOUBLE PRECISION XR0,XI0,FN
        ...
        DO L=1,NU
          DO WHILE(K.LT.N)
            DO I=1,N2
              ...
              K1=K+1
              K1N2=K1+N2
              TREAL=XR(K1N2)*CC+XI(K1N2)*SS
              TIMAG=XI(K1N2)*CC-XR(K1N2)*SS
              XR(K1N2)=XR(K1)-TREAL
              XI(K1N2)=XI(K1)-TIMAG
              XR(K1)=XR(K1)+TREAL
              XI(K1)=XI(K1)+TIMAG
...
C
C******************************************************************************
!NO HACE FALTA: YA ESTA DEFINIDA EN CFFT.
