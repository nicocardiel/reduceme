...
Comment
C
C SUBROUTINE GAUSCFIT_MOVEL(X0,SIGMA,AMP,Y0,EEX0,EESIGMA,EEAMP,EEY0,YRMSTOL)
C
C Input: YRMSTOL
...
C
C This subroutine is identical to GAUSCFIT, but the physical dimensions
C of the data to be fitted is defined to be NMAXFFT. This change is
C required for the program movel.
C
Comment
C------------------------------------------------------------------------------
        SUBROUTINE GAUSCFIT_MOVEL(X0,SIGMA,AMP,Y0,
     +   EEX0,EESIGMA,EEAMP,EEY0,YRMSTOL)
        IMPLICIT NONE
        INCLUDE 'redlib.inc'
        INTEGER NP
        REAL XF(NMAXFFT),YF(NMAXFFT)
        COMMON/BLKFITG1_MOVEL/NP
