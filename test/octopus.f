...
C
	...
	CHARACTER*50 CDUMMY
	CHARACTER*75 INFILE,MARKFILE,OUTFILE,FIBREFILE
        LOGICAL INSIDE1
...
700	  IF(NOBJ.GT.0)THEN
	    ...
	    WRITE(*,101) '(x) exit'
	    WRITE(*,100) 'Option (i/s/m/c/p/x) '
	    CLOCATE=READC('@','ismcpx')
	  ELSE
	    WRITE(*,101) '(i) Load fibre configuration from file'
	    WRITE(*,101) '(s) Save fibre configuration into file'
	    WRITE(*,101) '(c) Locate fibre to cursor'
