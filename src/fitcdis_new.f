C------------------------------------------------------------------------------
C Version 23-October-2003                                       file: fitcdis.f
C------------------------------------------------------------------------------
...
C
        ...
        INTEGER NORDER_ITER,NORDER,NTERMS,NTERMS0,NPTS,MODE,NEVAL
...
C
        ...
        END DO
C
15      WRITE(*,100)'No. of channels to search for line peaks '//
     +   '(integer, -=iterative search) '
        NSEARCH=READI('3')
        IF(NSEARCH.EQ.0)THEN
          WRITE(*,101) 'ERROR: this number must be different from zero'
          WRITE(*,101) 'Try again.'
          WRITE(*,100) 'Press <CR> to continue...'
          READ(*,*)
          GOTO 15
        ELSEIF(NSEARCH.LT.0)THEN
          NSEARCH=-NSEARCH
          LITERATIVE=.TRUE.
        ELSE
          LITERATIVE=.FALSE.
...
C
        ...
        DO I=1,NSCAN
          READ(14) (S(J,I),J=1,NCHAN)
        END DO
        CLOSE(14)
C
        WRITE(*,100)'Spectra from... to... '
        WRITE(CDUMMY,'(I1,A1,I6)')1,',',NSCAN
...
C
        ...
        END DO
C------------------------------------------------------------------------------
C Buscamos los maximos de cada linea de modo nuevo iterativo
        IF(LITERATIVE)THEN
          WRITE(CDUMMY,*) (IBEG+IEND)/2
          WRITE(*,100)'Initial scan to start iterative search'
          NS0=READILIM(CDUMMY,IBEG,IEND)
          WRITE(*,100)'Polynomial degree for iterative search'
          NORDER_ITER=READILIM('2',0,19)
c
          MM=0
          DO NS=NS0,IBEG,-1
            ...
            DO I=1,NLINES
              YMAX=0.
              IMAX=0
              IF(MM-1.GT.NORDER_ITER)THEN              !usamos nueva predicción
                DO J=1,MM-1
                  XPOL(J)=FLOAT(J)
                  YPOL(J)=XXPOS(I,J)
                  SIGMAY(J)=1.
     ...
     +               ' too near to the edge of the frame.'
          ...
          DO NS=NS0,IBEG,-1
            MM=MM+1
            DO I=1,NLINES
              XPOS(I,(NS0-IBEG+1)-MM+1)=XXPOS(I,MM)
            END DO
          END DO
c
          MM=0
     ...
     +               ' too near to the edge of the frame.'
          ...
          MM=IEND-IBEG+1
C..............................................................................
C Buscamos los maximos de cada linea de modo antiguo
        ELSE
          WRITE(*,*)
          MM=0
          DO MJ=IBEG,IEND
...
ccc           IPLOT=0
          ...
          IF(IPLOT.EQ.0) THEN
            IPL1=1
            WRITE(CDUMMY,'(A,I3,A,I4)')'C-distortion - Line #',I,
     +       '   Channel: ',LINE(I)
            DO ITERM=NTERM,1,-1
              CALL PGSLCT(IDN(ITERM))
              CALL PGENV(0.,REAL(NSCAN),YMIN,YMAX,0,1)
              CALL PGIDEN_RED
