C------------------------------------------------------------------------------
C Version 21-January-2000                                       file: fcalspl.f
C------------------------------------------------------------------------------
...
C Description: Generates a flux calibration spectrum using splines.
C
Comment
C
...
C
        DO ITERM=NTERM,1,-1
          ...
          IF(VSIGMA.GT.0.0)THEN
            ...
            CALL PGSCI(1)
          END IF
          CALL PGLABEL('Channel','No. of counts','file: '//DATFILE)
        END DO
C
        DO J=1,NCHAN
          S(J)=S0(J)
        END DO
C cargamos tabla con longitudes de onda y magnitudes
     ...
     +     'file: '//TABFILE(1:TRUELEN(TABFILE))
        ...
        DO ITERM=NTERM,1,-1
          CALL PGSLCT(IDN(ITERM))
          CALL PLOT(NPT,WV,FV,0,0,.FALSE.,0,2)
          CALL PGLABEL('Wavelength','Magnitude',
     +     'file: '//TABFILE(1:TRUELEN(TABFILE)))
        END DO
C
        DO I=1,NPT
          FV(I)=(10.**(-0.4*FV(I)))/WV(I)/WV(I)  !dividimos por l**2
        END DO                           !porque d(l)= c / (l**2) * d(nu)
C
        IF(WV(1)*RCVEL1.GT.STWV)THEN
          WRITE(*,101) 'FATAL ERROR: range covered by the table in '//
     +     'the BLUE is insufficient.'
          CALL PGEND
          STOP
        END IF
        IF(WV(NPT)*RCVEL1.LT.STWV+REAL(NCHAN-1)*DISP)THEN
          WRITE(*,101) 'FATAL ERROR: range covered by the table in '//
     +     'the RED is insufficient.'
          CALL PGEND
          STOP
        END IF
C normalizamos a un punto intermedio
        FDUMMY=FV(NPT/2)
        DO I=1,NPT
          FV(I)=FV(I)/FDUMMY
        END DO
C pintamos la tabla completa y el rango que nos interesa
        WRITE(*,100)'Press <RETURN>'
        READ(*,*)
        DO ITERM=NTERM,1,-1
          CALL PGSLCT(IDN(ITERM))
          CALL PLOT(NPT,WV,FV,0,0,.FALSE.,0,1)
          CALL PGLABEL('Wavelength','Normalized Flux',
     +     'file: '//TABFILE(1:TRUELEN(TABFILE)))
          IF(LCOLOR(ITERM)) CALL PGSCI(2)
          CALL PGMOVE(STWV,YMIN)
          CALL PGDRAW(STWV,YMAX)
          CALL PGMOVE(STWV+REAL(NCHAN-1)*DISP,YMIN)
          CALL PGDRAW(STWV+REAL(NCHAN-1)*DISP,YMAX)
          CALL PGRECT(STWV,STWV+REAL(NCHAN-1)*DISP,
...
C
        DO ITERM=NTERM,1,-1
          ...
          CALL PGMTEXT('T',3.0,0.,0.,DATFILE)
          IF(LCOLOR(ITERM)) CALL PGSCI(4)
          CALL PGBIN(NCHAN,XP,YPT,.TRUE.)
          CALL PGMTEXT('T',3.0,1.,1.,TABFILE)
          IF(LCOLOR(ITERM)) CALL PGSCI(5)
          CALL PGMTEXT('T',3.0,.5,.5,'Response curve')
...
C
          DO ITERM=NTERM,1,-1
            ...
            IF(LCOLOR(ITERM)) CALL PGSCI(3)
            CALL PGBIN(NCHAN,XP,YPS,.TRUE.)
            CALL PGMTEXT('T',2.5,0.,0.,DATFILE)
            IF(LCOLOR(ITERM)) CALL PGSCI(4)
            CALL PGBIN(NCHAN,XP,YPT,.TRUE.)
            CALL PGMTEXT('T',2.5,1.,1.,TABFILE)
            IF(LCOLOR(ITERM)) CALL PGSCI(5)
            CALL PGMTEXT('T',2.5,.5,.5,'Response curve')
            CALL PGBIN(NCHAN,XP,Y,.TRUE.)
            IF(LCOLOR(ITERM)) CALL PGSCI(NCOLOR)
            CALL PGBIN(NCHANBIN,XBIN,YBIN,.TRUE.)
            IF(NBIN.NE.1)THEN
...
C Si IYMAX=1 se fuerza a que YMAX no exceda de 20
