C------------------------------------------------------------------------------
C Version 6-December-1996                                    file: findmax_wl.f
C------------------------------------------------------------------------------
...
C Description: Automatic detection of line peaks in a spectrum and estimation
C of the peaks through the fit to a gaussian
C
Comment
C
...
C
        ...
        DO I=NMED+1,NCHAN-NMED-1
          ...
          END IF
        END DO
        WRITE(*,110)'No. of lines initially found: ',K
C
        WRITE(*,100)'Repeat initial search (y/n) '
        CREPEAT(1:1)=READC('n','yn')
        IF(CREPEAT.EQ.'y') GOTO 10
C
        WRITE(*,100)'Perform fit and save output into a file (y/n) '
        CSAVE(1:1)=READC('n','yn')
        IF(CSAVE.EQ.'y')THEN
          WRITE(*,100)'Output file name'
          OUTFILE=OUTFILEX(30,'@',0,0,.0,.0,3,.FALSE.)
          K=0
          WRITE(30,101) '#WAVELENGTH  I  X0  SIGMA  AMP'
          DO I=NMED+1,NCHAN-NMED-1
            LGOOD=.TRUE.
            J=0
            DO WHILE((J.LT.NMED).AND.(LGOOD))
            ...
            IF((S(I).GE.DATAMIN).AND.(LGOOD))THEN
              ...
              NSIMUL=0
              YRMSTOL=1E-6
              CALL GAUSSFIT(N,XFIT,YFIT,YFIT,X0,SIGMA,AMP,
     +                      EX0,ESIGMA,EAMP,
     +                      EEX0,EESIGMA,EEAMP,
     +                      YRMSTOL,NSIMUL)
              WRITE(30,*) STWV+(X0-1.0)*DISP,I,X0,SIGMA,AMP
              DO J=1,NCMAX
                XPLOT(J)=REAL(I-NMED)-.5+REAL(J-1)/REAL(NCMAX-1)*REAL(N)
                YPLOT(J)=AMP*EXP(-((XPLOT(J)-X0)*(XPLOT(J)-X0)/
     +           (2.*SIGMA*SIGMA)))
              END DO
              DO ITERM=NTERM,1,-1
                CALL PGSLCT(IDN(ITERM))
                IF(LCOLOR(ITERM)) CALL PGSCI(2)
                CALL PGLINE(NCMAX,XPLOT,YPLOT)
                IF(LCOLOR(ITERM)) CALL PGSCI(1)
              END DO
            END IF
          END DO
