C------------------------------------------------------------------------------
C Version 05-May-2004                                          file:writefits.f
C @ ncl & fjg
C------------------------------------------------------------------------------
Comment
C
...
C------------------------------------------------------------------------------
        OUTFILEX=OUTFILEX
C
        THISPROGRAM='writefits'
        CALL WELCOME('05-May-2004')
...
10      WRITE(*,100)'Output file name? '
        READ(*,'(A)')OUTFILE
        IF(LRUN)THEN
          CALL RMBLANK(OUTFILE,OUTFILE,L2)
          WRITE(*,101) OUTFILE(1:L2)
        END IF
        IF(LMANUAL)THEN
          CALL RMBLANK(OUTFILE,OUTFILE,L2)
          WRITE(*,101) '\\ttshade{'//OUTFILE(1:L2)//'}'
        END IF
        IF(LHTML)THEN
          CALL RMBLANK(OUTFILE,OUTFILE,L2)
          WRITE(*,101) '<FONT COLOR="#FF0000">'//OUTFILE(1:L2)//
     +     '</FONT>'
        END IF
...
C write REDUCEME required keywords
     ...
     +     IYEAR
          ...
          CALL FTPKYF(UNIT,'CRPIX1',1.0,2,' ',ISTATUS)
     ...
     +     'linear dispersion (Angstrom/pixel)',ISTATUS)
          ...
          CALL FTPKYS(UNIT,'FITSfi',FITSFILE,'FITS file name',ISTATUS)
...
20        WRITE(*,100) 'Header template file name? '
          READ(*,'(A)') TEMPFILE
          IF(LRUN)THEN
            CALL RMBLANK(TEMPFILE,TEMPFILE,L2)
            WRITE(*,101) TEMPFILE(1:L2)
          END IF
          IF(LMANUAL)THEN
            CALL RMBLANK(TEMPFILE,TEMPFILE,L2)
            WRITE(*,101) '\\ttshade{'//TEMPFILE(1:L2)//'}'
          END IF
          IF(LHTML)THEN
            CALL RMBLANK(TEMPFILE,TEMPFILE,L2)
            WRITE(*,101) '<FONT COLOR="#FF0000">'//TEMPFILE(1:L2)//
     +       '</FONT>'
          END IF
